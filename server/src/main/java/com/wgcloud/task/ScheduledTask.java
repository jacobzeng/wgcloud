...
public class ScheduledTask {
     ...
     * 线程池
     */
    static ThreadPoolExecutor executor = new ThreadPoolExecutor(10, 40, 2, TimeUnit.MINUTES,
    ...
    @Autowired
    NetAdapterStateService netAdapterStateService;
            ...
            if (!CollectionUtil.isEmpty(list)) {
                ...
                for (AppInfo appInfo : list) {
                    ...
                    if (diff > delayTime) {
                        ...
                        logInfo.setInfoContent("超过10分钟未上报状态，可能已下线IP：" + appInfo.getHostname() + "，名称：" + appInfo
                                .getAppName() + "，进程ID：" + appInfo.getAppPid());
                        logInfo.setState(StaticKeys.LOG_ERROR);
                        logInfoList.add(logInfo);
            ...
            if (heathMonitorAllList.size() > 0) {
                for (HeathMonitor h : heathMonitorAllList) {
                    ...
                    if (!"200".equals(h.getHeathStatus())) {
                        ...
                        logInfo.setInfoContent(
                                "服务接口检测异常：" + h.getAppName() + "，" + h.getHeathUrl() + "，返回状态" + h.getHeathStatus());
                        logInfo.setState(StaticKeys.LOG_ERROR);
                        logInfoList.add(logInfo);
                        Runnable runnable = () -> {
            ...
            for (DbInfo dbInfo : dbInfos) {
                ...
                for (DbTable dbTable : dbTables) {
                    ...
                    if ("postgresql".equals(dbInfo.getDbType())) {
                        sql = RDSConnection.query_table_count_pg
                                .replace("{tableName}", dbTable.getTableName()) + whereAnd + dbTable.getWhereVal();
                    } else {
                        sql = RDSConnection.query_table_count
                                .replace("{tableName}", dbTable.getTableName()) + whereAnd + dbTable.getWhereVal();
                    }
    ...
    public synchronized void commitTask() {
        ...
        try {
            ...
            if (BatchData.NET_ADAPTER_STATE_LIST.size() > 0) {
                Map<String, Object> paramsDel = new HashMap<String, Object>();

                List<NetAdapterState> NET_ADAPTER_STATE_LIST = new ArrayList<NetAdapterState>();
                NET_ADAPTER_STATE_LIST.addAll(BatchData.NET_ADAPTER_STATE_LIST);
                BatchData.NET_ADAPTER_STATE_LIST.clear();
                List<String> hostnameList = new ArrayList<String>();
                for (NetAdapterState netAdapterState : NET_ADAPTER_STATE_LIST) {
                    if (!hostnameList.contains(netAdapterState.getHostname())) {
                        hostnameList.add(netAdapterState.getHostname());
                    }
                }
                for (String hostname : hostnameList) {
                    paramsDel.put("hostname", hostname);
                    netAdapterStateService.deleteByAccHname(paramsDel);
                }
                netAdapterStateService.saveRecords(NET_ADAPTER_STATE_LIST);
            }
            ...
            if (BatchData.APP_INFO_LIST.size() > 0) {
                ...
                for (AppInfo systemInfo : APP_INFO_LIST) {
                    ...
                    for (AppInfo systemInfoS : savedList) {
                        if (systemInfoS.getHostname().equals(systemInfo.getHostname()) && systemInfoS.getAppPid()
                                .equals(systemInfo.getAppPid())) {
                            systemInfo.setId(systemInfoS.getId());
                            updateList.add(systemInfo);
                            issaved = true;
                            break;
    ...
    public void clearHisdataTask() {
        ...
        try {
            ...
            //执行删除操作begin
            if (paramsDel.get(StaticKeys.SEARCH_END_TIME) != null && !""
                    .equals(paramsDel.get(StaticKeys.SEARCH_END_TIME))) {
