...
public class SigarUtil {
   ...
   /* public static void printCpuPerc(CpuPerc cpu) {
    ...
    public static List<NetAdapterState> ethernet(Timestamp t) throws Exception {
        List<NetAdapterState> list = new ArrayList<NetAdapterState>();
        String[] ifaces = sigar.getNetInterfaceList();
        for (int i = 0; i < ifaces.length; i++) {
            String name = ifaces[i];
            try {
                NetAdapterState netAdapterState = new NetAdapterState();
                netAdapterState.setHostname(commonConfig.getBindIp());
                NetInterfaceConfig cfg = sigar.getNetInterfaceConfig(name);
                netAdapterState.setName(name);
                netAdapterState.setCreateTime(t);
                netAdapterState.setIpv4Address(cfg.getAddress());
                netAdapterState.setIpv4Netmask(cfg.getNetmask());
                list.add(netAdapterState);
